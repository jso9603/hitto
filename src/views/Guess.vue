<template>
  <div class="guess" ref="guessRef">
    ...
    <button class="challenge" :disabled="isButtonDisabled" @click="onChallenge">
...
export default class Guess extends Vue {
  ...
  checkButtonDisabled() {
    const now = dayjs()
    const saturdayEightPM = dayjs().day(6).hour(18).minute(0).second(0) // 이번 주 토요일 20:00
    const saturdayMidnight = saturdayEightPM.add(6, 'hours') // 토요일 18:00에서 6시간 후 (즉, 자정)

    // 현재 시간이 토요일 20:00부터 자정(6시간) 사이인지 확인
    this.isButtonDisabled =
      now.isAfter(saturdayEightPM) && now.isBefore(saturdayMidnight)
  }
